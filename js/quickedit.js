(function($) {
	var wp_inline_edit  = inlineEditPost.edit; // we create a copy of the WP inline edit post function

	inlineEditPost.edit = function(id) { // and then we overwrite the function with our own code
		...
		if ( post_id > 0 ) {
			...
			var memb_anyloggedinuser  = jQuery('#memb-anyloggedinuser-' + post_id ).val();
			...
			if (memb_anyloggedinuser == 1) {
				edit_row.find('#memb_anyloggedinuser').attr('checked', 'checked');
			}
