...
<?php if ( $has_downloads ) : ?>
	...
	<table class="woocommerce-MyAccount-downloads shop_table shop_table_responsive">
		<thead>
			<tr>
				<?php 
                $table_columns = wc_get_account_downloads_columns();
				$columns = array();
				foreach ($table_columns as $key => $value) {
					if ($key == 'download-product') {
					 $columns[$key] = 'Product - (Download)';
					} else {
                       $columns[$key] = $value;
					}
				}
					...
					<td class="<?php echo esc_attr( $column_id ); ?>" data-title="<?php echo esc_attr( $column_name ); ?>">
						...
						<?php elseif ( 'download-product' === $column_id ) : ?>
							<a href="<?php echo esc_url( get_permalink( $download['product_id'] ) ); ?>">
								<?php echo esc_html( $download['product_name'] ).' - ('.esc_html( $download['download_name'] ).')'; ?>
							</a>
                            <?php 
                             $order_license_code = get_post_meta($download['order_id'],'provided_license_key',true);
						        $has_license = '';
						        if ( metadata_exists( 'post', $download['product_id'], 'csmultiplecheckout_product_license' ) && !empty($order_license_code) ) {
						                $if_product_license = array('');
						                $if_product_license = get_post_meta($download['product_id'],'csmultiplecheckout_product_license',true);
						                if (!empty($if_product_license)) {
						                    $key_arr = array('');
						                   foreach ($if_product_license as $key => $value) {
						                     if ($value['key'] == $order_license_code) {
						                         $has_license = $value['key'];
						                     }
						                   }
						                }         
						        }
						        if (!empty($has_license)) {
						         echo '<span class="download_item_license_number">Activation Code: '.$order_license_code.'</span>';
						        }
							?>
