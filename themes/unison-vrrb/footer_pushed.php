    ...
    if (username != '' && password != '') {
        jQuery.ajax({
            ...
            success: function(response) {
                ...
                if (response == 'damntrue') {
                    //location.reload();
                    window.location.href = "/my-account";
                }
                else if (response == 'nosubscription') {
                    location.reload();
                    //window.location.href = "/my-account";
                } else {
                    //jQuery('.my_cs_error_msg').remove();
        ...
        <div
            ...
            <div
                ...
                <ul class="row col-xxxl-12  col-xxl-12  col-xl-12 col-lg-12  col-md-12 col-xs-12 col-sm-12 m-0">
                    <div class="col-xxxl-7 col-xxl-7 col-xl-7 col-lg-7  col-md-12 col-xs-12 col-sm-12 pl-0 pr-0">
                        ...
                        <li class="text-white text-left align-items-center"> <img class="img-fluid"
                                src="<?php bloginfo('template_url')?>/assets/images/correct green.png"> 48 Chord
                            Progressions</p>
                    </div>
                    ...
                    <div class="col-xxxl-5 col-xxl-5 col-xl-5 col-lg-5 col-md-12 col-xs-12 col-sm-12 pl-0 mt-10-new">
                        <li class="text-white text-left align-items-center"> <img class="img-fluid"
                                src="<?php bloginfo('template_url')?>/assets/images/correct green.png"> 48 Basslines
                        </li>
                    </div>
                    <div class="col-xxxl-7 col-xxl-7 col-xl-7 col-lg-7 col-md-12 col-xs-12 col-sm-12 mt-10 pl-0">
                        <li class="text-white text-left align-items-center"><img class="img-fluid"
                                src="<?php bloginfo('template_url')?>/assets/images/correct green.png"> 48 Melodies</li>
                    </div>
                    <div class="col-xxxl-5 col-xxl-5 col-xl-5 col-lg-5 mt-10 col-md-12 col-xs-12 col-sm-12 pl-0">
                        <li class="text-white text-left align-items-center"><img class="img-fluid"
                                src="<?php bloginfo('template_url')?>/assets/images/correct green.png"> 56 Drum Kits
                        </li>
                    </div>
                </ul>
                 <?php
                ...
                <div
                    ...
                    <div class="row">
                        ...
                        <p
                            ...
                            TODAYS TOTAL: <span class="big-green-text"><?php echo $pprrcc; ?> </span>
                        </p>
                    </div>
                </div>
                <div class="text-left col-xxxl-12 col-xxl-12 col-xl-12 col-lg-12 col-md-12 col-xs-12 col-sm-12 p-0"
                    ...
                    <!-- <form class="downgarde-plan" method="post" enctype="multipart/form-data" id="downgarde-plan">
                        <input type="hidden" name="add-to-cart" value="290493">
                        <input type="hidden" id="variation" name="variation_id" value="290496">
                        <button type="submit" class="badge badge-pill badge-success btn-shadow btn-upgrade" id="downgrade-sub">Downgrade to lite plan</button>
                    </form> -->
                </div>
...
$(document).ready(() => {
    ...
    $('#cancel-sub').click((event) => {
        ...
        if(getProduct.includes('lite') || getProduct.includes('Lite')){
            ...
            $('.loading-reactivate').removeClass('d-block');
        }else{
            $('#modal-overlay').removeClass('closed');
...
function populateSavedCrads(){   
    ...
    $('#payment #place_order').click(function(){
...
<script>
    jQuery(document).ready(function(){
        if(jQuery('#gform_5')[0]){
         ...
         jQuery('.button.subbutreglog').click(function(e){
             ...
             if(/^[a-zA-Z0-9- ]*$/.test(regusername) == false) {
                if(!jQuery('.charerr')[0]){
                    jQuery('p.login_fields').append('<div class="error charerr"><strong>ERROR: Characters are not allowed.<br></strong></div>');
                }
                return false;
             }
             else{
                jQuery('.error.charerr').remove();
             }
             if(regusername.length > 15){
                if(!jQuery('.limierr')[0] != ''){
                    jQuery('p.login_fields').append('<div class="error limierr"><strong>ERROR: Please limit your username to 15 characters.<br></strong></div>');
                }
                return false;
             }
             else{
                jQuery('.error.limierr').remove();   
             }
             
             var regupass = jQuery('.regupass').val();
             var regumail = jQuery('.regumail').val();
             var sign_up_newsletter = jQuery('.sign_up_newsletter').val();
			 jQuery('.button.subbutreglog p').text('Loading...');
             jQuery.ajax({
                 url: afp_vars.afp_ajax_url,
                 method: "post",
                 data: {
                 ...
                 success: function(response) {
					 jQuery('.button.subbutreglog p').text('create account');
                     if('0' != response){
                         if(jQuery('.error')[0]){
                             jQuery('.error').remove();
    ...
    // if(jQuery('#modal-cart')[0]){
    //     if(jQuery('#modal-cart').hasClass('closed')){
    //     }else{
...
$(document).ready(function() {
    ...
    if($('.card_section_sub').length){
        $('.card_section_sub img').addClass('mastercard-image');
    }

    if(window.location.href.split('/').includes('view-subscription') || window.location.href.split('/').includes('my-account')){
        if($('.woocommerce-error').length){
            $('.woocommerce-error').hide();
        }
    }else{
        var myVar = setInterval(findError, 1000);
        function findError() {
            if($('.woocommerce-error').length){
                var err = $('.woocommerce-error li').first().text();
    ...
    if(localStorage.getItem('changeStatus') !== null && localStorage.getItem('changeStatus') == 'new'){ 
        $.ajax({url: prev, type:'GET',success:function(resp){
            localStorage.removeItem("changeStatus");
    ...
    $('#place_order').click(function(e){
        e.preventDefault();
        var FormDataCP = $('#order_review').serializeArray();
        var getStatus = '';
        $.map(FormDataCP,function(data, i){
            if(data.name.includes('payment-token')){
                getStatus = data.value;
            }
        });
        if(localStorage.getItem('changeStatus') === null){
            localStorage.setItem('changeStatus',getStatus);
        }else{
            localStorage.setItem('changeStatus',getStatus);
        }
        $('#order_review').submit();
    ...
    // one click purchase iframe
    $("#modal-cart iframe").on("load", function () {
        var payPal_select = false;
        var iframe = $('#modal-cart iframe');
        var selectPay = iframe.get(0).contentWindow.document.getElementsByClassName('select-pay');
        $(selectPay).click(function(){
           payPal_select = false;
        });
        var selectPaypal = iframe.get(0).contentWindow.document.getElementById('payment_method_select');
        $(selectPaypal).click(function(){
            payPal_select = true;
        });
        var cancelelmnt = iframe.get(0).contentWindow.document.getElementById('cancel-switching');
        $(cancelelmnt).click(function(){
            $('#modal-cart-close').click();
        });
        var confirmelmnt = iframe.get(0).contentWindow.document.getElementById('confirm-switching');
        $(confirmelmnt).click(function(){
            var checkoutElmnt = iframe.get(0).contentWindow.document.getElementsByClassName('complete-payment-button');
            checkoutElmnt[0].click();
            if(!payPal_select){
                $("#modal-cart").addClass('closed');
                $('.loading-reactivate').removeClass('d-none');
                $('.loading-reactivate').addClass('d-block');
            }
        });
    });

    $('#payment_method_select').click(function(){
        if($('#payment_method_stripe').is(':checked')){
            $('#payment_method_stripe').removeAttr('checked');
            $('#payment_method_paypal').attr('checked','checked');
            $('#checkout_form_switching').attr('target','_top');
        }
    });
    $('.select-pay').click(function(){
        if(!$('#payment_method_stripe').is(':checked')){
            $('#payment_method_paypal').removeAttr('checked');
            $('#payment_method_stripe').attr('checked','checked');
            $('#checkout_form_switching').attr('target','_parent');
        }
        var token = $(this).data('token');
        $('input[name="wc-stripe-payment-token"]').val(token);
    });
